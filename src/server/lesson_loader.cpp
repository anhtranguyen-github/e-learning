...
std::string Lesson::serializeMetadata() const {
    std::ostringstream oss;
    oss << lessonId << "|" << title << "|" << topic << "|" << level;
    return oss.str();
}
...
// ============================================================================
// LessonLoader Implementation
// ============================================================================

LessonLoader::LessonLoader(std::shared_ptr<Database> database) : db(database) {
}

std::vector<std::string> LessonLoader::parseJsonArray(const std::string& jsonStr) const {
...
bool LessonLoader::parseLessonFromRow(PGresult* result, int row, Lesson& lesson) const {
    try {
...
Lesson LessonLoader::loadLessonById(int lessonId) {
    Lesson lesson;
...
LessonList LessonLoader::loadLessonsByFilter(const std::string& topic, const std::string& level) {
    LessonList lessonList;
