...
ExamLoader::ExamLoader(std::shared_ptr<Database> database) : db(database) {
}

std::vector<std::string> ExamLoader::parseJsonArray(const std::string& jsonStr) const {
...
bool ExamLoader::parseExamFromRow(PGresult* result, int row, Exam& exam) const {
    try {
...
Exam ExamLoader::loadExamById(int examId) {
    Exam exam;
...
ExamList ExamLoader::loadExamsByFilter(const std::string& type, const std::string& level, int lessonId) {
    ExamList examList;
