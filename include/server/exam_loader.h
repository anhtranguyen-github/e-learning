...
// Loader class for loading exams from database
class ExamLoader {
private:
...
public:
    ExamLoader(std::shared_ptr<Database> database);
    ...
    // Load exams filtered by type, level, and/or lessonId
    ExamList loadExamsByFilter(const std::string& type, const std::string& level, int lessonId);
};
